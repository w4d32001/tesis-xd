...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \node[inner sep=0pt] (imagen) {\includegraphics[width=0.85\textwidth]{pipeline.png}};
        \draw[line width=3.5pt] (imagen.north west) rectangle (imagen.south east);
    \end{tikzpicture}
...
Cuestionario de usabilidad (SUS) & Eficiencia operativa del sistema & Puntuación normalizada SUS (0--100) & \parencite{Afian2025} \\
\hline
...
\begin{axis} [
    ...
    \node[color=blue!70!black] at (axis cs:-1.0,0.11) {\footnotesize ($1 - \alpha = 0.95$)};
    
    \node[color=red!80!black, align=center] at (axis cs:4.0,0.08) {\footnotesize \textbf{Región de}\\\footnotesize \textbf{Rechazo ($\alpha = 0.05$)}};
    
    % Flecha indicadora para el área de rechazo
    \draw[<-] (axis cs:3.5,0.02) -- (axis cs:4.0,0.05);
