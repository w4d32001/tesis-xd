...
%====================================================
¿En cuánto disminuye la sobrecarga de apuntes y la fatiga mental, y en qué medida se optimiza el rendimiento académico de los estudiantes universitarios de la UNAMBA al transicionar desde esquemas de estudio aislados hacia un entorno de aprendizaje interconectado y automatizado?
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \node[inner sep=0pt] (imagen) {\includegraphics[width=0.45\textwidth]{image/unamba.png}};
        \draw[line width=3.5pt] (imagen.north west) rectangle (imagen.south east);
    \end{tikzpicture}
    \caption{Universidad Nacional Micaela Bastidas de Apurímac (UNAMBA).}
    \label{fig:logo_unamba}
\end{figure}