...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \draw[line width=3.5pt] (current bounding box.north west) rectangle (current bounding box.south east);
    \end{tikzpicture}
...
\noindent\begin{tabular}{@{} p{1cm} l p{8.0cm} @{}}
  & $Q, K, V$ & representan las matrices de \textit{queries}, \textit{keys} y \textit{values} proyectadas; \\
  & $d_k$ & es la dimensión de las \textit{keys}, cuya raíz cuadrada actúa como factor de escala para estabilizar los gradientes \parencite{Vaswani2017}.
\end{tabular}

La extensión multi-cabeza (\textit{Multi-Head Attention}) ejecuta múltiples operaciones de atención en paralelo sobre distintos subespacios de representación, permitiendo al modelo capturar simultáneamente dependencias sintácticas, semánticas y de largo alcance en el texto \parencite{Vaswani2017}.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
        \draw[line width=3.5pt] ($(current bounding box.north west)+(-5pt,5pt)$) rectangle ($(current bounding box.south east)+(5pt,-5pt)$);
    \end{tikzpicture}
